...
### 训练
- **连续录入(练一组记一组)**:选器械 → 默认 1 行 stepper(自动沿用上一组的重量/次数)→ 点「保存本组」即落库,弹层不关,自动重置为下一行;顶部绿色横幅实时显示「✓ 已记 N 组」。全套练完点「完成」或 ×,若当前行你编辑过会自动保存(没动过的空默认行不会误存)。
- 仍可点「+ 添加一组」在一次打开里录多行(用于补录历史)。
- 历史里点「改」仍是整存整取编辑(一次修改一条 entry 的所有组)。
- 器械库:在「设置」页里管理(预存器械:名称 + 部位 + 有重量/自重),记录时直接选,不用重复输入。
...
### 历史
- 按日期合并展示训练 + 练习 + 热量,可改可删。
- 顶部 chip 切换「全部 / 训练 / 练习 / 热量」单独查看某一类。

### 设置(右上角齿轮)
- **器械库**:新增 / 编辑 / 删除器械,一键载入 14 个常用示例。
- **数据管理**:导出 JSON 备份、导入 JSON、清空所有数据。
...
> 注意:
> - 不同浏览器/设备的 localStorage 不共享,手机上是空数据。要搬数据:电脑「设置 → 数据管理 → 导出 JSON」→ 传到手机 → 手机「设置 → 数据管理 → 导入 JSON」。
...
- 报表「训练 / 练习 / 热量」切换而非同图叠加(量纲不同);历史 tab 同样提供「全部/训练/练习/热量」chip 筛选。
...
- **训练录入走"练一组记一组"的连续模式**:默认 1 行、保存本组即落库、弹层不关、上一组重量自动带下,符合真实训练组间休息时随手记的节奏;「+ 添加一组」保留补录场景。数据模型仍一条 entry 包含多组(跨几次保存累积),报表/历史/聚合逻辑零改动。
- 器械库和数据管理收进右上角齿轮「设置」页,底部 navbar 只留「记录/报表/历史」三个高频入口。
